...
$sentencia = $base_de_datos->prepare("SELECT * FROM productos WHERE codigo = ? and sucursal_pro = 'Las brisas' LIMIT 1;");
$sentencia->execute([$codigo]);
$producto = $sentencia->fetch(PDO::FETCH_OBJ);
# Si no existe, salimos y lo indicamos
if (!$producto) {
    header("Location: ./vistaventasjoya.php?status=4");
    exit;
}
# Si no hay existencia...
if ($producto->existencia < 1) {
    header("Location: ./vistaventasjoya.php?status=5");
    exit;
}
session_start();
